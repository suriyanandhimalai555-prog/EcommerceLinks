...
- **Income model (since migration 020 — "2 Direct Pair Matching"):** a *pair* completes at member P when **both of P's direct referrals are active** (leg balance across the deep subtree is irrelevant; the old `min(L,R) − pairs_matched` counter-minting is gone). Each completed pair accrues ₹1000 (`CFG.PAIR_BONUS_PAISE`) to P **and to every placement ancestor of P** (`pair_accruals`, one row per beneficiary). Accruals stay `pending` until the beneficiary is **qualified** (3-gen gate, since July 2026: **both** directs active + ≥1 active grandchild under an active direct; `pair_accruals.release_seq` versions the release idempotency key so a qualification revert can re-release), then release to the wallet retroactively via `creditBonusWithCap` (per-cutoff cap applies). **Since migration 036 the weekly-cap overage is FORFEITED, not deferred:** `creditBonusWithCap` credits the wallet up to the ₹1 lakh weekly cap and the excess to the system `bonus_forfeited` account (it never reaches the member, but stays auditable) — including a large qualification-backlog release, which forfeits everything above ₹1 lakh in that single cutoff. The legacy defer-and-carry path (`sweepDeferred`/`DeferredSweepRequested`, `deferred_bonus` account) is left intact only so any pre-036 deferred balances still drain out; no new deferred credits are created, so it self-deactivates. Detection: `workers/pairComplete.ts`; upline fanout: `workers/fanout.ts`; accrual/release: `workers/ledger.ts`; release trigger: `PendingBonusReleaseRequested` written by `evaluateQualification`. `member_counters.pairs_matched` is deprecated (frozen at 0).

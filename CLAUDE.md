...
**Modular monolith.** Fastify API (`avg-api`) takes writes and records every pipeline-relevant state change in the `events_outbox` table inside the same DB transaction. One worker process (`avg-workers`, `workers/all.ts`) runs eight loops: the outbox relay publishes outbox rows to **Redis Streams** (`XADD`); consumers (`fanout`, `counterPair`, `qualification`, `ledger`, `rank`) read via consumer groups (`XREADGROUP`/`XACK`/`XAUTOCLAIM`) and are idempotent through `processed_events`; three timer loops (`cutoff`, `payout`, `reconciler`) run internal Asia/Kolkata schedulers. **Postgres is the source of truth and the durable event log; Redis is transport + rate limiting — losing Redis loses no data.** Deployment: Railway (Singapore) — `avg-api` + `avg-workers` + Postgres/Redis plugins; frontend is a static Vite build on Cloudflare Pages. Full spec: PLAN.md §2A and §5.

**Build state:** the transport migration (PLAN.md §2A) is **complete**. `lib/streams.ts` is the transport; `workers/all.ts` consolidates all eight loops. The legacy broker files (kafka.ts, createTopics.ts, docker-compose.yml, kafkajs) have been deleted. CI and Railway deployment are the next steps (PLAN.md Phase 7 steps ① and ⑧).
...
npm run dev:workers           # ALL eight worker loops in one process (tsx, hot-reload)
...
- **Events:** any state change that must notify the pipeline calls `writeOutbox(c, event)` **inside the same transaction**. Never `XADD` to a stream directly from API routes — only `outboxRelay` publishes (sanctioned exceptions: fanout's direct `publishToStream` of counter increments, and the admin DLQ replay endpoint re-delivering an already-published event). New event types go in `events/types.ts` + a routing case in `events/outbox.ts`. Stream keys and consumer groups are in PLAN.md §2A.
- **Income model (since migration 038 — "Carry-forward Pair Matching" v2):** every node maintains cumulative counts of active members in its left and right subtrees (`member_counters.left_active`/`right_active`). `counterPair.ts` sets `pairs_matched = LEAST(left_active, right_active)` in a single UPDATE per ancestor per batch — **set-to-target, never a decrement** (D-3). Pairs are minted one per matched unit (`INSERT INTO pairs … ON CONFLICT (member_id, sequence_no) DO NOTHING`) and are unbounded — no one-per-member cap. Each completed pair accrues ₹1000 (`CFG.PAIR_BONUS_PAISE`) to the **matching node only** — no ancestor fan-out (BR-7); `pair_accruals` has exactly one row per pair. Accruals stay `pending` until the earner is **qualified** (3-gen gate: **both** directs active + ≥1 active grandchild under an active direct; `pair_accruals.release_seq` versions the idempotency key so a revert can re-release), then release via `creditBonusWithCap`. **Cap modes (BR-12/BR-13):** immediate releases (already-qualified earner, ongoing accrual) forfeit cap overage to `bonus_forfeited`; qualification-backlog releases defer overage to the member's `deferred_bonus` account, which is swept each cutoff close via `sweepDeferred`/`DeferredSweepRequested`. Detection and minting: `workers/counterPair.ts`; accrual/release: `workers/ledger.ts`; release trigger: `PendingBonusReleaseRequested` written by `evaluateQualification`. `workers/pairComplete.ts` is deleted (pair matching is now inside `counterPair.ts`); `workers/fanout.ts` no longer handles `PairCompleted`.
...
- **Two trees.** `sponsor_id` (who referred you — used ONLY for the 3-generation qualification gate) vs `parent_id`+`position` (binary placement — used for all counters/pairs/ranks). Picking the wrong tree gives wrong numbers, not errors. **Since the 2-referral cap** (July 2026): registration places the new member directly under their sponsor, so `parent_id = sponsor_id` for all new rows and the trees coincide going forward — but both columns remain, and pre-cap data may still have `sponsor_id ≠ parent_id`. **Placement side:** `POST /auth/register` takes an optional `leg` (`L`/`R`). Omitted → auto-fill (first referral → L, second → R, third → 409 "Referral limit reached") — this is the member's generic share link and the `simulate.ts`/test path. Sent → the recruit is **pinned** to that slot, or **409 "Left/Right position already filled"** if it is taken (never silently reassigned to the other leg). Tapping a vacant slot in the tree copies a leg-specific link (`?sponsor=CODE&leg=L|R`); `resolvePosition` in `services/placement.ts` enforces both cases under the sponsor `FOR UPDATE` lock.
...
- **Runtime feature flags live in `system_settings` (migration 022/023).** A management-controlled key/value (`key`, `value jsonb`) table read at request time and flipped via `PATCH /admin/settings` (audit-logged) — **no redeploy** changes behaviour. Current flags: `kyc_optional` (default `false` = KYC mandatory for `POST /orders`), `login_otp_enabled` (default `false`), `welcome_email_enabled` (default `false`). If OTP/KYC/welcome-mail behaviour "suddenly changed", check the flag before the code — the same build behaves differently per-environment.
- **Auth has two optional/secondary flows on top of password login.** (1) **Login OTP** — when `login_otp_enabled` is on, login returns an email OTP challenge; verification is single-use, expiry-bounded, rate-limited, and **management accounts bypass OTP entirely** so the master account is never locked out. (2) **Password reset** — scoped-OTP flow with email delivery (`01cfd8f`), reusing the OTP infra (no dedicated migration). Frontend surfaces an OTP entry step with expiry countdown + resend cooldown.
- **Notifications: feed is client-derived, read-state is server-tracked.** The list itself is computed in `frontend/src/lib/useNotifications.ts` from wallet credits + achieved ranks (no backend feed table). But **unread state** is server-persisted via `members.notifications_seen_at` (migration 030): `POST /me/notifications/seen` bumps it, `/me` returns `notificationsSeenAt`, and a notification is read iff its `at <=` that timestamp — so clearing the bell is cross-device consistent. Don't reintroduce client-only "read" state.
- **Order status includes `rejected`; payment proofs are multi-image.** `orders.status` CHECK is `created`/`paid`/`confirmed`/`refunded`/`failed`/`rejected` (migration 027) with a `rejection_reason` column. Management can reject a payment proof (member re-uploads) or **override `rejected → paid`**. Payment-proof screenshots live in `order_payment_proofs` (one order → many S3 keys, migration 025); the older single `orders.payment_proof_key` column (024) was dropped (026). `rejected` orders are reusable by `createOrder` dedup — see the product-deletion gotcha above.
- **`bank_status` is three-state** (`pending`/`verified`/`rejected`, migration 031), mirroring `kyc_status`. Members manage their own bank details; admins can reject with a reason, forcing re-submission. Payout eligibility still requires `bank_status = 'verified'`.
- **Members share phones; login is email-only.** `members_phone_key` was dropped (migration 029) — families/groups register under one mobile number. Phone stays `NOT NULL` (contact/display) but is **not an identity key**; `POST /auth/login` and all lookups use email (which stays UNIQUE, lowercased). Do not re-add the phone unique constraint. **Placement-tree depth reads are capped at `CFG.MAX_TREE_DEPTH`** (env `MAX_TREE_DEPTH`, default 12; raised from 6, `0e4756a`).
- **Products are flat-priced — no GST.** GST was removed (`c62e1be`); the order path applies no tax. A `GST_PCT` env var / `CFG.GST_PCT` may still exist in config and older `INTEGRATION.md` snippets, but it is **unused on the live order path** — don't wire it back in.
...
- **`writeOutbox` must stay inside the caller's transaction**; producers other than `outboxRelay` should not publish lifecycle/ledger events directly (sanctioned exceptions: fanout's direct `publishToStream` of counter increments, and the admin DLQ replay re-delivering an already-published event).

  ...
  await withTxn(async (c) => {
    ...
    for (const inc of fresh) {
      if (inc.counter_type === 'active') {
        ...
        if (inc.side === 'L') { leftQual++;  qualChanged = true }
        else                  { rightQual++; qualChanged = true }
      } else if (inc.counter_type === 'rank_achiever' && inc.rank_level !== undefined) {
        const side = inc.side
        if (side === 'L') {
          await c.query(
          ...
          await c.query(
            `INSERT INTO leg_rank_counters (member_id, rank_level, right_count)
             VALUES ($1,$2,0) -- placeholder; conflict update handles it
             ON CONFLICT (member_id, rank_level)
    ...
    for (let k = 1n; k <= newPairs; k++) {
      ...
      if (!la[0] || !ra[0]) {
        throw new Error(`Missing leg_activation for ancestor=${ancestorId} seq=${seq} — data integrity error`)
      }
      ...
      const { rows: pairRows } = await c.query<{ id: string }>(
        `INSERT INTO pairs (member_id, sequence_no, left_member_id, right_member_id, bonus_amount)
         ...
         RETURNING id`,
        [ancestorId, seq, la[0].member_id, ra[0].member_id]
      )
      if (pairRows[0]) {
        await writeOutbox(c, {
          event_id:        randomUUID(),

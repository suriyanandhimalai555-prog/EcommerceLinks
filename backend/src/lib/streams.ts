...
function publisher(): Redis {
  if (!_publisher) {
    ...
    _publisher.on('error', (err: Error) => console.error('[streams] publisher error', err))
  }
  return _publisher
}

export async function publishToStream(stream: string, value: string): Promise<void> {
...
export async function startConsumer(opts: ConsumerOpts): Promise<void> {
  ...
  conn.on('error', (err: Error) => console.error(`[streams:${group}] error`, err))
    ...
    if (Date.now() - lastClaim > 60_000) {
      try {
        ...
        const claimed = await (conn as any).call(
          'XAUTOCLAIM', stream, group, consumer, '60000', '0-0', 'COUNT', String(count)
        ) as [string, XEntry[], string[]]
        const reclaimed: XEntry[] = claimed[1] ?? []
        if (reclaimed.length > 0) {
          await dispatch(conn, stream, group, mode, reclaimed, onMessage, onBatch)
        }
      } catch (err) {
  ...
  if (mode === 'message') {
    for (const [id, fields] of entries) {
      ...
      try {
        ...
        console.error(`[streams:${group}] handler error (id=${id}), leaving pending`, err)
        continue
      }
    ...
    const values = entries.map(([, fields]) => extractValue(fields)).filter(Boolean) as string[]
    try {
      await onBatch!(values)
    } catch (err) {
      // Leave all pending — XAUTOCLAIM will redeliver the whole batch.
      console.error(`[streams:${group}] batch handler error, leaving pending`, err)
      return
    }

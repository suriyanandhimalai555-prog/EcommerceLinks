	...
	switch (e.event_type) {
		...
		case "CounterIncrement":
			return {
				...
				partitionKey: String(e.ancestor_id),
				aggregateType: "member",
		...
		case "CutoffClosed":
			return {
				topic: TOPICS.payouts.name,
				partitionKey: String(e.cutoff_id),
				aggregateType: "cutoff",
				aggregateId: BigInt(e.cutoff_id),
			};
		case "PayoutBatchCreated":
			return {
				topic: TOPICS.payouts.name,
				partitionKey: String(e.batch_id),
				aggregateType: "payout_batch",
				aggregateId: BigInt(e.batch_id),
			};
		case "PayoutItemSettled":
		case "PayoutItemFailed":
			return {
				topic: TOPICS.payouts.name,
				partitionKey: String(e.payout_item_id),
				aggregateType: "payout_item",
				aggregateId: BigInt(e.payout_item_id),
			};
	}
}
